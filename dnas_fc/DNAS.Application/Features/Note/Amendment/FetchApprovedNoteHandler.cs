...
{
    public class FetchApprovedNoteCommand(NoteModel note): IRequest<MyApprovedNoteModel>
    {
        public NoteModel _note { get; set; } = note;
    }
    internal sealed class FetchApprovedNoteHandler(INote iNote, ICustomLogger logger, IEncryption encryption, IHttpContextAccessor haccess) : IRequestHandler<FetchApprovedNoteCommand, MyApprovedNoteModel>
    {
        ...
        public async Task<MyApprovedNoteModel> Handle(FetchApprovedNoteCommand request, CancellationToken cancellationToken)
        {
            ...
            {                
                ...
                Response = await _iNote.FetchApprovedNote(inparam);                
                if (Response != null)
                {
                    ...
                    _logger.LogwriteInfo("FetchApprovedNoteCommand data fetch successfully done", loginUserId);
                    return Response;
                }
                else
                {
