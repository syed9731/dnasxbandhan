...
{
    public class InsertNoteExpensesIncurredAtCommand(NoteModel note) : IRequest<NoteModel>
    {
        public NoteModel _note { get; set; } = note;
    }
    internal sealed class InsertNoteExpensesIncurredAtHandler(ISave iISave, ICustomLogger logger, IEncryption iEncryption, IHttpContextAccessor haccess) : IRequestHandler<InsertNoteExpensesIncurredAtCommand, NoteModel>
    {
        private readonly ISave _iISave = iISave;
        public readonly ICustomLogger _logger = logger;
        private readonly IEncryption _iEncryption = iEncryption;
        private readonly string loginUserId = $"User_{haccess.HttpContext?.User.FindFirstValue("UserId")}";
        public async Task<NoteModel> Handle(InsertNoteExpensesIncurredAtCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Response = await _iISave.InsertExpenseIncuredAtData(note);
