...
{
    public class FetchFyiNoteDetailsCommand(NoteModel note): IRequest<ViewFyiNoteModel>
    {
        public NoteModel _note { get; set; } = note;
    }

    internal sealed class FetchFyiNoteDetailsHandler(INote iNote, ICustomLogger logger, IEncryption encryption, IHttpContextAccessor haccess) : IRequestHandler<FetchFyiNoteDetailsCommand, ViewFyiNoteModel>
    {
        private readonly INote _iNote = iNote;
        public readonly ICustomLogger _logger = logger;
        private readonly IEncryption _encryption = encryption;
        private readonly string loginUserId = $"User_{haccess.HttpContext?.User.FindFirstValue("UserId")}";
        public async Task<ViewFyiNoteModel> Handle(FetchFyiNoteDetailsCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Response = await _iNote.FetchFyiNoteData(inparam);
                if (Response != null)
                {
