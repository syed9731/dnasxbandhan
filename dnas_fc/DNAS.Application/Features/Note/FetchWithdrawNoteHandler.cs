...
{
    public class FetchWithdrawNoteCommand(NoteModel note): IRequest<WithdrawNoteModel>
    {
        public NoteModel _note { get; set; } = note;
    }
    internal sealed class FetchMyApprovedNoteHandler(INote iNote, ICustomLogger logger, IEncryption encryption, IHttpContextAccessor haccess) : IRequestHandler<FetchWithdrawNoteCommand, WithdrawNoteModel>
    {
        ...
        public async Task<WithdrawNoteModel> Handle(FetchWithdrawNoteCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Response = await _iNote.FetchWithdrawNote(inparam);                
                if (Response != null)
                {
                ...
                else
                {
                    ...
                    return new WithdrawNoteModel();
                }
                
            }
            catch (Exception ex)
            {
                _logger.LogwriteInfo("exception occur during FetchWithdrawNoteHandler execution--- message-" + Environment.NewLine+ex.Message+Environment.NewLine+ex.StackTrace, loginUserId);
                return Response;
            }
