...
{
    public class InsertNoteBodyCommand(NoteModel note) : IRequest<NoteModel>
    {
        public NoteModel _note { get; set; } = note;
    }
    internal sealed class InsertNoteBodyHandler(ISave iISave, ICustomLogger logger, IEncryption iEncryption, IHttpContextAccessor haccess) : IRequestHandler<InsertNoteBodyCommand, NoteModel>
    {
        private readonly ISave _iISave = iISave;
        public readonly ICustomLogger _logger = logger;
        private readonly IEncryption _iEncryption = iEncryption;
        private readonly string loginUserId = $"User_{haccess.HttpContext?.User.FindFirstValue("UserId")}";
        public async Task<NoteModel> Handle(InsertNoteBodyCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                note.NoteBody = request._note.NoteBody;
                note.UserId = request._note.UserId;
                Response = await _iISave.InsertNoteBodyData(note);
