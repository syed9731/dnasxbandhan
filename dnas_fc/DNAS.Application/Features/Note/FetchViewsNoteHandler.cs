...
{
    public class FetchViewsNoteCommand(NoteModel note): IRequest<ViewsNoteModel>
    {
        public NoteModel _note { get; set; } = note;
    }
    internal sealed class FetchViewsNoteHandler(INote iNote, ICustomLogger logger, IEncryption encryption, IHttpContextAccessor haccess) : IRequestHandler<FetchViewsNoteCommand, ViewsNoteModel>
    {
        ...
        public async Task<ViewsNoteModel> Handle(FetchViewsNoteCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Response = await _iNote.FetchViewsNote(inparam);                
                if (Response != null)
                {
