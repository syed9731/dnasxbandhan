...
{
    public class InsertRecomendedApproverByNoteIdCommand(NoteModel note) : IRequest<NoteModel>
    {
        public NoteModel _note { get; set; } = note;
    }
    internal sealed class InsertRecomendedApproverByNoteIdHandler(ICustomLogger logger, IEncryption iEncryption, IDapperFactory iDapperFactory) : IRequestHandler<InsertRecomendedApproverByNoteIdCommand, NoteModel>
    {
        ...
        public async Task<NoteModel> Handle(InsertRecomendedApproverByNoteIdCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                PorcFetchRecomendedApproverByNoteIdInput InParams = new()
                {
                    @NoteId = note.NoteId,
                ...
                if (!string.IsNullOrEmpty(request._note.ApproverIdList))
                {
                    ...
                    ApproverForDraftModel DbResult = await _iDapperFactory.ExecuteSpDapperAsync<ApproverForDraft, ApproverForDraftModel>(
                        SpName: OraStoredProcedureNames.ProcFetchRecomendedApproverByNoteId, Params: InParams);
                ...
                else
                {
                    ...
                    _ = await _iDapperFactory.ExecuteSpDapperAsync<ApproverForDraft, ApproverForDraftModel>(
                        SpName: OraStoredProcedureNames.ProcDeleteLastApprover, Params: InParams);
                    return Response = new();
                }
            }
            catch (Exception ex)
