(function(factory) {
  ...
  let dom = $.summernote.dom;

  // define the popover plugin
  let DataBasicPlugin = function(context) {
    let self = this;
    ...
    self.initialize = function() {
      ...
      let $container = options.dialogsInBody ? $(document.body) : context.layoutInfo.editor;
      ...
      let body = '<div class="form-group row-fluid">' +
          ...
          '</div>';
      let footer = '<button href="#" class="btn btn-primary ext-databasic-save">' + lang.databasic.insert + '</button>';
      ...
      let $content = self.$popover.find('.popover-content');
    ...
    self.update = function() {
      ...
      let rng = context.invoke('editor.createRange');
      let visible = false;

      if (rng.isOnData()) {
        let $data = $(rng.sc).closest('data.ext-databasic');

        if ($data.length) {
          let pos = dom.posFromPlaceholder($data[0]);
    ...
    self.createNode = function(info) {
      let $node = $('<data class="ext-databasic"></data>');
    ...
    self.openDialog = function(info) {
      return $.Deferred(function(deferred) {
        let $inpTest = self.$dialog.find('.ext-databasic-test');
        let $saveBtn = self.$dialog.find('.ext-databasic-save');
